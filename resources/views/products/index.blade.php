...
@section('scriptsAfterJs')
  <script>
    ...
    $(document).ready(function () {
      ...
      $('.search-form select[name=order]').val(filters.order).on('change', function(){
        //解析当前查询参数
    ...
    //定义一个函数，用于解析当前Url里的参数，并以key-value对象形式返回
    function parseSearch(){
      //初始化一个空对象
      var searches = {};
      //location.search会返回URL中?以及后面的查询参数
      //substr(1)将?去除，然后以符号&分割成数组，然后遍历这个数组
      location.search.substr(1).split('&').forEach(function(str){
        //将字符串以符号=分割成数组
        var result = str.split('=');
        //将数组的第一个值解码之后作为key，第二个值解码之后作为value放到之前初始化的对象中
